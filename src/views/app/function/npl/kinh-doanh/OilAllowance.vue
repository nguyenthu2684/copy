        ...
        <b-tabs v-model="currentTab" nav-class="d-none">
          ...
          <b-tab>
                ...
                <div class="d-flex flex-wrap">
                    ...
                    <b-row>
                      ...
                      <b-col lg="6">
                        <div class="align-items-center">
                          <div
                            class="style-special"
                            style="white-space: inherit; margin-right: 5px"
                          >
                          ...
                          <div style="max-width: 100%">
                            <b-form-input
                              class="text-right"
                              v-model="allowanceTruckWithCrane"
                            ></b-form-input>
                          </div>
                        </div>
                      </b-col>
                  ...
                  <b-col lg="4">
                    <div class="style-special">
                      *Mức phụ cấp trong tải/chuyến
                    </div>
                    <b-table
                      :items="tempData.dataType"
                      :fields="tempData.fieldsType"
                      :bordered="true"
                      show-empty
                      head-variant="light"
                    >
                      <template #cell(allowance)="row">
                        <div class="d-flex align-items-center">
                          <div style="max-width: 50px" class="text-center">
                            <b-form-input
                              v-model="row.item.allowance"
                              class="text-right"
                            />
                          </div>
                          <div class="ml-1">lít</div>
                        </div>
                      </template>
                      <template #empty>
                  ...
                  <b-col lg="4">
                    <div>
                      <div class="style-special" style="white-space: wrap">
                        *Mức phụ cấp quá tải/chuyến
                      </div>
                      <b-table
                        :items="tempData.dataType2"
                        :fields="tempData.fieldsType"
                        :bordered="true"
                        show-empty
                        head-variant="light"
                ...
                <div class="d-flex flex-wrap mt-4">
                  <b-col lg="6" class="pl-0">
                    <div class="table-relative">
                      <div class="d-flex flex-wrap align-items-center mb-2">
                        <div class="style-special">
                          *Mức phụ cấp theo xe số lít/100km
                        </div>
                        <!-- <b-button
                          variant="primary"
                          class="d-inline-block ml-2 btn-show-modal-select"
                          size="sm"
                          @click="showModalSelect('vehicle')"
                          >Chọn</b-button
                        > -->
                        <b-button
                          size="sm"
                        ...
                        <template #cell(delete)="row">
                          <div class="text-center btn-remove">
                            <div
                              class="text-danger"
                              @click="
                                removeSelectedVehicle(row.item.licensePlates)
                              "
                            >
                      ...
                      <b-pagination
                        align="center"
                        :total-rows="selectedVehicle.length"
                  ...
                  <b-col lg="6" class="pr-0">
                    <div class="table-relative">
                      <div class="d-flex flex-wrap align-items-center mb-2">
                        <div class="style-special">
                          *Mức phụ cấp theo tuyến đường
                        </div>
                        <!-- <b-button
                          size="sm"
                          variant="primary"
                          class="d-inline-block ml-2 btn-show-modal-select"
                          @click="showModalSelect('route')"
                          >Chọn</b-button
                        > -->
                        <b-button
                          size="sm"
                      ...
                      >
                        <template #cell(delete)="row">
                          <div class="text-center btn-remove">
                            <div
                              class="text-danger"
                              @click="removeSelectedRoute(row.item.ID)"
                            >
                              <i class="fad fa-trash-alt"></i>
                            </div>
                          </div>
                      ...
                      <b-pagination
                        align="center"
                        :total-rows="selectedRoute.length"
            ...
            <!-- <div class="box-content" v-if="false">
              <div v-if="title" style="font-size: 14px" class="mt-1 mb-2">
                <strong>NHIỆP VỤ : {{ title.toUpperCase() }}</strong>
                <hr class="m-0" />
              </div>
              <b-row v-if="fieldsTable && fieldsTable.length > 0">
                <template v-for="(value, i) in fieldsTable">
              ...
              <b-row >
                <b-col lg="6">
                  <b-form-group label="Biển số xe">
                    <b-form-select :options="tempData.licensePlatesOptions" v-model="formModalAdd.licensePlates"/>
                  </b-form-group>
                </b-col>
                <b-col lg="6">
                  <b-form-group label="Loại xe">
                    <b-form-input v-model="formModalAdd.type" disabled/>
                  </b-form-group>
                </b-col>
                <b-col lg="4">
                  <b-form-group label="Định mức cũ">
                    <b-form-input v-model="formModalAdd.oldLimit" disabled class="text-right"/>
                  </b-form-group>
                </b-col>
                <b-col lg="4">
                  <b-form-group label="Định mức mới">
                    <b-form-input v-model="formModalAdd.newLimit" class="text-right"/>
                  </b-form-group>
                </b-col>
                <b-col lg="4">
                  <b-form-group label="Chênh lệch">
                    <b-form-input v-model="formModalAdd.deviant" disabled class="text-right"/>
                  </b-form-group>
                </b-col>
              </b-row>
            </template>
              ...
              <b-row >
                <b-col lg="12">
                  <b-form-group label="Tuyến đường">
                    <b-form-select :options="tempData.routeOptions" v-model="formModalAdd.route"/>
                  </b-form-group>
                </b-col>
                <b-col lg="4">
                  <b-form-group label="Định mức cũ">
...
import tempData from "@/data/modules/npl/oilAllowanceDependVehicleProposal(1.2)";
...
export default {
  ...
  data() {
    return {
      ...
      keyComponent: "basic",
      //title-options
      currentTitle: 'Định mức tiêu hao và phụ cấp dầu theo xe' ?? null,
      ...
      ////////
      tempData: tempData,
      title: "tiêu hao và phụ cấp dầu theo xe",
      isLock: false,
      perPageTable2: 10,
      currentPageTable2: 1,
      perPageTable3: 10,
      ...
      titleOption: [
        {
          id: 0,
          label: "Định mức tiêu hao và phụ cấp dầu theo xe",
        },
      ],
      modalAddName: '',
      titleModalAdd: '',
  ...
  watch: {
    'formModalAdd.licensePlates'(newVal) {
      const obj = this.tempData.dataLicensePlatesInfo.find(item => item.licensePlates == newVal)
      this.formModalAdd.type = obj ? obj.type : ''
      this.formModalAdd.oldLimit = obj ? obj.oldLimit : ''
    },
    'formModalAdd.newLimit'(newVal) {
      this.formModalAdd.deviant = this.calcDeviant(newVal, this.formModalAdd.oldLimit)
    },
    'formModalAdd.route'(newVal) {
      const obj = this.tempData.dataRoute.find(item => item.ID == newVal)
      this.formModalAdd.oldLimit = obj ? obj.oldLimit : ''
    }
  },
  methods: {
    ...
    showModalAdd(name) {
      ...
      if(name == 'section1') {
        this.formModalAdd = {}
        this.titleModalAdd = 'Thêm phương tiện'
      }
      else if(name == 'section2') {
        this.formModalAdd = {}
        this.titleModalAdd = 'Thêm tuyến đường'
      }
