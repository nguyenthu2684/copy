...
export default {
  ...
  methods: {
    ...
    addModal() {
      ...
      this.typeForm = "ADD";
      this.apiForm = "/api/SLLightLevels/Add";
    ...
    editModal(items) {
      this.getDataByID(items[0].LightID);
      setTimeout(() => {
        ...
        this.typeForm = "EDIT";
        this.apiForm = "/api/SLLightLevels/Edit";
        this.bodyFormDefault = {
          ID: items[0].LightID,
          MenuIDCurent: this.objectData.menuIDCurrent,
        };
    ...
    deleteModal(items) {
      this.$bvModal
        .msgBoxConfirm(this.$t("form.question") + items[0].LightName + "?", {
        ...
        .then((value) => {
          if (value === true) {
            this.handleSubmitDelete(items[0].LightID);
          }
        })
        .catch((err) => {
          console.log(err);
  ...
  async created() {
    ...
    await this.getColumn("SLLightLevel");
    await this.getKeyTable();
    await this.getData();
    await this.getFormAddShow("SLLightLevel");
  },
  watch: {
    items() {
      this.getColumn("SLLightLevel");
